...
public class SSDatabaseSqlite { // Default SQLite Database interface
    private static final String downloadURL = "http://cloud.github.com/downloads/wargamer/SignShop/";
    private static Driver driver = null;
    private Connection conn = null;
    private String filename;

    public SSDatabaseSqlite(final String pFilename) {
        filename = pFilename;
    ...
    public Boolean tableExists(String tablename) {
        try {
            ...
            ResultSet set = (ResultSet) runSqliteStatement("SELECT name FROM sqlite_master WHERE type = ? AND name = ?;", pars, true);
    ...
    public boolean columnExists(String needle) {
        ResultSet result = (ResultSet) runSqliteStatement("PRAGMA table_info(Book);", null, true);
        if(result == null)
            return false;
    ...
    public Object runSqliteStatement(String Query, Map<Integer, Object> params, Boolean expectingResult) {
        try {
            if(conn == null) {
